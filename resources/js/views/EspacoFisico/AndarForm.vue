            ...
            <div class="row g-3">
              ...
              <div class="col-md-4">
                ...
                <input 
                  ...
                  required
                />
                <small class="text-white-50">Use números negativos para subsolo (-1, -2)</small>
              </div>
              ...
              <div class="col-md-12">
                ...
                <textarea 
                  ...
                  class="form-control bg-transparent text-white border-secondary"
                  rows="3"
                  placeholder="Descrição do andar..."
                ></textarea>
              </div>

              <!-- Infraestrutura -->
              <div class="col-md-12">
                <div class="section-header-mini">Infraestrutura</div>
              </div>
...
const props = defineProps({
  andar: { type: Object, default: null }
});
...
onMounted(async () => {
  ...
  if (props.andar) {
    Object.assign(form, props.andar);
    // Se tem bloco, carregar a hierarquia completa
    if (props.andar.bloco) {
      // Buscar instituição através do campus
      if (props.andar.bloco.predio?.campus?.instituicao_id) {
        form.instituicao_id = props.andar.bloco.predio.campus.instituicao_id;
        await carregarCampi();
      }
      
      // Campus
      if (props.andar.bloco.predio?.campus_id) {
        form.campus_id = props.andar.bloco.predio.campus_id;
        await carregarPredios();
      }
      
      // Prédio
      if (props.andar.bloco.predio_id) {
        form.predio_id = props.andar.bloco.predio_id;
...
const salvar = async () => {
  ...
  try {
    ...
    if (props.andar) {
      await axios.put(`/api/v1/andares/${props.andar.id}`, payload);
    } else {
      await axios.post('/api/v1/andares', payload);
    }
    emit('success');
  } catch (error) {
    console.error('Erro ao salvar:', error);
    Swal.fire('Erro!', error.response?.data?.message || 'Erro ao salvar andar', 'error');
  } finally {
