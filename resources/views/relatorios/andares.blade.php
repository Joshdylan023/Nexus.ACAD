...
<head>
    ...
    <title>Relatório de Andares</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="header-wrapper">
        <table class="header-table">
            ...
            <tr>
                ...
                <td class="info-col">
                    <p><strong>Total de andares:</strong> {{ $andares->count() }}</p>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th style="width: 18%;">BLOCO</th>
                <th style="width: 18%;">PRÉDIO</th>
                <th style="width: 16%;">CAMPUS</th>
                <th style="width: 10%;" class="text-center">ESPAÇOS</th>
        ...
        <tbody>
            ...
            <tr>
                <td class="text-center"><strong>{{ $andar->numero }}</strong></td>
                <td><strong>{{ $andar->nome }}</strong></td>
                <td>{{ $andar->bloco?->nome ?? '-' }}</td>
                <td>{{ $andar->bloco?->predio?->nome ?? '-' }}</td>
                <td>{{ $andar->bloco?->predio?->campus?->nome ?? '-' }}</td>
                <td class="text-center">{{ $andar->espacosFisicos->count() }}</td>
                <td>
                    <span class="badge badge-{{ strtolower($andar->status ?? 'ativo') }}">
                        {{ $andar->status ?? 'Ativo' }}
                    </span>
                </td>
            </tr>
            ...
            <tr>
                <td colspan="7" style="text-align: center; padding: 30px; color: #999;">
                    Nenhum andar encontrado com os filtros aplicados.
                </td>
            </tr>
            @endforelse
        </tbody>
